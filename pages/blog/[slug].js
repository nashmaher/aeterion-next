...
function renderContent(content) {
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith('## ')) {
      ...
      elements.push(<p key={i} style={{ fontWeight: 700, fontSize: 16, color: '#1B3A6B', margin: '20px 0 8px' }}>{line.slice(2, -2)}</p>)
    } else if (line.startsWith('- ')) {
      elements.push(<li key={i} style={{ fontSize: 15, color: '#374151', lineHeight: 1.75, marginBottom: 6 }}>{renderInline(line.slice(2))}</li>)
    } else if (line.trim() === '') {
        ...
        <main style={{ maxWidth: 780, margin: '0 auto', padding: '0 24px 80px' }}>
          <article style={{ background: '#fff', borderRadius: 16, border: '1px solid #e5e7eb', padding: '48px', marginBottom: 32 }}>
            ...
            <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'wrap', alignItems: 'center' }}>
              <span style={{ background: cat.bg, color: cat.color, padding: '4px 12px', borderRadius: 20, fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5 }}>
                {post.category}
              </span>
            ...
            {/* Title */}
            <h1 style={{ margin: '0 0 20px', fontSize: 32, fontWeight: 900, color: '#1B3A6B', lineHeight: 1.25, letterSpacing: -0.5 }}>
              {post.title}
            </h1>

            {/* Excerpt */}
            <p style={{ margin: '0 0 32px', fontSize: 17, color: '#6B7280', lineHeight: 1.7, paddingBottom: 32, borderBottom: '1px solid #F3F4F6' }}>
              {post.excerpt}
            </p>
            ...
            <div style={{ marginTop: 48, padding: '20px 24px', background: '#FEF3C7', borderRadius: 12, border: '1px solid #FDE68A' }}>
              ...
              <div style={{ fontSize: 13, color: '#92400E', lineHeight: 1.7 }}>
                All Aeterion Labs products are sold strictly for laboratory research purposes only. They are not intended for human consumption, medical treatment, or therapeutic use. Not evaluated by the FDA. Must be 18+ to purchase.
              </div>
            </div>
          </article>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 16 }}>
                {relatedPosts.map(rp => (
                  <a key={rp.slug} href={`/blog/${rp.slug}`} style={{ textDecoration: 'none' }}>
                    <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e5e7eb', padding: '20px', cursor: 'pointer' }}>
                      ...
                      <div style={{ fontSize: 15, fontWeight: 700, color: '#1B3A6B', lineHeight: 1.4, marginBottom: 8 }}>{rp.title}</div>
